!function (a) {
    ...
    a.prototype = {
        init: function () {
            ...
            dug({endpoint: "https://api.instagram.com/v1/users/1140781/media/recent/?client_id=01a35e4dd4bf4d2681e48c88d006636e&count=6", target: jQuery("#beta-instagram-feed")[0], cacheExpire: 3600, template: '<div style="display: none">            {{#data}}                <a href="{{link}}" target="_blank">                  <img src="{{images.low_resolution.url}}">                </a>            {{/data}}</div>'})
        }, initFilter: function () {
            jQuery.fn.slider && (jQuery("#price-filter-range").slider({
                range: !0, min: 0, max: 1e3, values: [0, 300], slide: function (a, b) {
